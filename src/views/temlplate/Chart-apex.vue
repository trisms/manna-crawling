	...
	methods: {
		...
		getChart1Options() {
			return {
				...
				colors: [appVariable.color.blue, appVariable.color.gray800],
				dataLabels: {
					enabled: true,
					background: {
						enabled: true,
				...
				grid: {
					row: { colors: ['rgba('+ appVariable.color.componentColorRgb + ', .25)', 'transparent'], opacity: 0.5 }
				},
				markers: { size: 4 },
				xaxis: { 
			...
			return {
				...
				yaxis: {
					title: {
						text: '$ (thousands)',
						style: {
							color: 'rgba('+ appVariable.color.gray500Rgb + ', .5)',
							fontSize: '12px',
		...
		getChart4Options() {
			return {
				...
				dataLabels: {
					enabled: true,
					offsetX: -6,
					style: { fontSize: '12px', colors: [appVariable.color.gray500] }
				},
		...
		getChart5Options() {
			return {
				...
				dataLabels: { enabled: false },
				stroke: { width: [0, 0, 3] },
				colors: [appVariable.color.blue, 'rgba('+ appVariable.color.gray500Rgb + ', .5)', appVariable.color.orange],
				...
				yaxis: [
					{ 
						axisTicks: { show: true, }, 
						axisBorder: { show: true, color: appVariable.color.blue }, 
						labels: { style: { color: appVariable.color.blue } },
						title: { text: "Income (thousand crores)", style: { color: appVariable.color.blue } }, 
						tooltip: { enabled: true }
					},
					{
						seriesName: 'Income', opposite: true,
						axisTicks: { show: true, },
						axisBorder: { show: true, color: 'rgba('+ appVariable.color.gray500Rgb + ', .5)' },
						labels: { style: { color: 'rgba('+ appVariable.color.gray500Rgb + ', .5)' } },
						title: { text: "Operating Cashflow (thousand crores)", style: { color: 'rgba('+ appVariable.color.componentColor + ', .5)' } },
					}, 
					{
						seriesName: 'Revenue',
						opposite: true,
	...
	data() {
		return {
			...
			chart7: {
				height: 350,
				series: [
					{ name: 'Bubble1', data: this.generateBubbleChartData(new Date('11 Feb 2022 GMT').getTime(), 20, { min: 10, max: 60 }) },
					{ name: 'Bubble2', data: this.generateBubbleChartData(new Date('11 Feb 2022 GMT').getTime(), 20, { min: 10, max: 60 }) },
					{ name: 'Bubble3', data: this.generateBubbleChartData(new Date('11 Feb 2022 GMT').getTime(), 20, { min: 10, max: 60 }) },
					{ name: 'Bubble4', data: this.generateBubbleChartData(new Date('11 Feb 2022 GMT').getTime(), 20, { min: 10, max: 60 }) }
				],
